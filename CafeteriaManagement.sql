...
(
	...
	Points INT NOT NULL DEFAULT 0,               -- Điểm tích lũy của khách hàng
	MembershipLevel NVARCHAR(100) NOT NULL DEFAULT N'Bronze' -- Hạng thành viên
)
GO
...
-- Customer Loyalty
CREATE PROCEDURE UpdateMembershipLevel
    @CustomerID INT
AS
BEGIN
    DECLARE @Points INT
    SELECT @Points = Points FROM Customer WHERE CustomerID = @CustomerID

    IF @Points < 100
        UPDATE Customer SET MembershipLevel = N'Bronze' WHERE CustomerID = @CustomerID
    ELSE IF @Points >= 100 AND @Points < 500
        UPDATE Customer SET MembershipLevel = N'Silver' WHERE CustomerID = @CustomerID
    ELSE IF @Points >= 500 AND @Points < 1000
        UPDATE Customer SET MembershipLevel = N'Gold' WHERE CustomerID = @CustomerID
    ELSE
        UPDATE Customer SET MembershipLevel = N'Diamond' WHERE CustomerID = @CustomerID
END
GO

-- Call Procedure
DECLARE @CustomerID INT
SELECT @CustomerID = CustomerID FROM OrderData WHERE OrderID = SCOPE_IDENTITY()
EXEC UpdateMembershipLevel @CustomerID

-- TEST Procedure
SELECT CustomerName, PhoneNumber, Points, MembershipLevel 
FROM Customer